...
prop.change <- 0.4
# (Fig. 3a and b were created with prop.change = 0.4 and 0.8)
...
prop.increase <- 0.5
# (Fig. 3a and b were created with prop.increase 0.5
#  Fig. 3c and d were created with prop.increase = 0.2 and 0.8)
...
# (Fig. 3a, b, c and d were created with decrease_selection_weighted = 0, 
#  i.e. no weight,
#  Fig. 3e and f were created with decrease_selection_weighted = 1,
...
for(i in 1:100){
  ...
  if(no.species.inc>0){
    ...
    ## FMS changed -- broken in my version of dplyr
    #x.inc3 <- x.inc %>%
    #  slice_sample(n=0)
    x.inc3 <- x.inc[0,]
    ## end of FMS change
...
# Do the calculations by community
## FMS some more comments could be useful here
for (i in 1:100){
  print(i)
...
legend("topleft", bty="n", cex=1,
       ...
       legend=c("Species with negative mean change", 
                "Species with positive mean change"))
